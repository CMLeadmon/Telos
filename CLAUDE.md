...
**Frontend.** Next.js 16.2.10 static export (`output: "export"` in `next.config.ts`) — no server components at runtime, no API routes. Per-route client components under `frontend/src/app/(shell)/` (`chat`, `stream`, `library`, `files`, `settings`) inside `components/AppShell.tsx`, plus Zustand stores in `frontend/src/stores/` (15 of them — `useAuthStore`, `useThemeStore`, `useChatSessionStore`, `useMediaStore`, `useLibraryStore`, `useFilesStore`, `useAnnotationStore`, `usePreferencesStore`, `useSettingsStore`, `useMobileNavStore`, …). Shared design tokens (breakpoints 720/900/1100, fluid type, spacing, 44px touch targets, safe-area) live in `styles/tokens/scale.css`. In dev, the app detects port 3000 and points API/WS calls at `:8080`. Heed `frontend/AGENTS.md`: this Next.js version is newer than training data — consult `node_modules/next/dist/docs/` before nontrivial Next.js work, and never hand-edit `styles/tokens/` or `styles/foundations/` (imported verbatim from the design project).
